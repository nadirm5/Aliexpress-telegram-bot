...
def _build_response_message(product_data: dict, generated_links: dict, details_source: str) -> str:
    ...
    discounted_price = product_data.get('discounted_price')  # Prix réduit
    discount_percentage = product_data.get('discount_percentage')  # Pourcentage de réduction

    print(f"Product Title: {product_title}")
    print(f"Product Price: {product_price} {product_currency}")
    print(f"Discounted Price: {discounted_price} {product_currency} ({discount_percentage}% off)")
    ...
    if details_source == "API" and product_price:
        ...
        # Affichage du prix réduit, si disponible
        if discounted_price:
            discounted_str = f"{discounted_price} {product_currency}".strip()
            message_lines.append(f"💥 <b>Price After Discount:</b> {discounted_str} (Discount: {discount_percentage}%)\n")
